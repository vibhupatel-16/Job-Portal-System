...
export const googleCallback = async (req, res) => {
  ...
  try {
    ...
    if (tokens.refresh_token) {
      ...
      return res
        .status(400)
        .send("Failed to get refresh token. Please try again.");
    }
  } catch (error) {
...
export const scheduleInterview = async (req, res) => {
  try {
    const { applicationId, jobseekerId, date, time, mode, meetingLink } =
      req.body;
    ...
    if (existingInterview) {
      return res.status(400).json({
        message:
          "An interview is already scheduled for this application. You cannot schedule another interview.",
        success: false,
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Interview scheduling failed", success: false });
  }
};
...
export const getScheduledInterviewsByCreator = async (req, res) => {
  try {
    ...
    const myScheduledInterviews = interviews.filter(
      (item) => item.job !== null,
    );
...
export const approveReschedule = async (req, res) => {
  try {
    ...
    // ⭐ LIMIT RESCHEDULING: Check if interview has already been rescheduled once
    if (interview.rescheduleCount >= 1) {
      return res.status(400).json({
        message:
          "This interview has already been rescheduled once. Further rescheduling is not allowed.",
        success: false,
      });
    }
    ...
    if (isNaN(startDateTime.getTime())) {
      return res
        .status(400)
        .json({ message: "Invalid Date/Time received", success: false });
    }
...
export const deleteInterview = async (req, res) => {
  try {
    ...
    if (isJobseeker) cancellerType = "candidate";

    // Notification to jobseeker (if not the canceller)
    if (!isJobseeker) {
      await Notification.create({
        recipient: interview.jobseeker._id,
        sender: userId,
        ...
        message: `Your scheduled interview has been cancelled by the ${cancellerType}.`,
        link: "/jobseeker/interviews",
      });
    }

    // Notification to employer (if not the canceller)
    if (!isOwner) {
      await Notification.create({
        recipient: interview.scheduledBy._id,
        sender: userId,
        type: "STATUS_UPDATED",
        title: "Interview Cancelled",
    ...
    // Notification to admin (always notify admin for status changes)
    ...
    for (const admin of admins) {
      await Notification.create({
        ...
        type: "STATUS_UPDATED",
        title: "Interview Cancelled",
        message: `Interview for ${interview.job?.title} at ${interview.company?.name} cancelled by ${cancellerType}.`,
...
export const submitFeedback = async (req, res) => {
  try {
    ...
    const overallRating =
      (ratings.technical + ratings.communication + ratings.cultureFit) / 3;
    ...
    if (req.io) {
      req.io
        .to(interview.jobseeker.toString())
...
export const getBookedSlots = async (req, res) => {
  try {
    ...
    if (!targetEmployerId) {
      return res
        .status(400)
        .json({ message: "Employer ID is required", success: false });
    }
