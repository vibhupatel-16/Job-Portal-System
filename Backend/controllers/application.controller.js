...
export const getAiMatchScore = async (req, res) => {
  try {
    ...
    const application =
      await Application.findById(applicationId).populate("job applicant");
    ...
    if (!resumeUrl)
      return res
        .status(400)
        .json({ message: "Resume URL not found", success: false });
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Could not read PDF.", success: false });
    }
...
export const applyJob = async (req, res) => {
  try {
    ...
    if (!applicant || !applicant.profile?.resume) {
      return res.status(400).json({
        message: "Please upload your resume in your profile before applying to jobs.",
        success: false,
      });
    }
...
export const getAnalyticsData = async (req, res) => {
  try {
    ...
    const statusData = [
      ...
      {
        name: "Shortlisted",
        value: allApps.filter((a) => a.status === "shortlisted").length,
      },
      {
        name: "Accepted",
