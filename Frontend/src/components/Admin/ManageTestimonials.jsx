...
const ManageTestimonials = () => {
  ...
  const employerOnly = useMemo(() => items.filter((t) => (t?.role || "").toLowerCase() === "employer"), [items]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-gray-50 to-indigo-50 px-6 py-10">
      <div className="flex items-end justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Employer Testimonials</h1>
          <p className="text-sm text-gray-600 mt-1">
            Yaha sirf <b>Employer</b> feedback show ho raha hai. Approve karte hi landing page par appear ho jayega.
          </p>
        </div>
        <button
          onClick={loadPending}
      ...
      ) : employerOnly.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-200 p-10 text-center text-gray-500 font-semibold">
          No pending employer testimonials.
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {employerOnly.map((t) => (
            <div key={t._id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
